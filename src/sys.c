...
void getos(struct comp_info *buf)
{
	...
	while (fgets(buffer, sizeof(buffer), file)) {
		...
		if (strncmp(buffer, "VERSION_ID=", 11) == 0) {
			...
			snprintf(version, sizeof(version), "%.*s", (int)len_v,
				 str_v);
		}
	...
	if (name[0] != '\0') {
		if (version[0] != '\0')
			snprintf(final_name, sizeof(final_name), "%s %s", name,
				 version);
		else
...
void power(struct comp_info *charge, struct comp_info *predict)
{
	...
	if (strcmp(status, "Charging") == 0) {
		if (rem_hour > 0)
			asprintf(&predict->sys_attr.b_ch_predict,
				 "(%dh %dm to full)", rem_hour, rem_min);
		else
			asprintf(&predict->sys_attr.b_ch_predict,
				 "(%dm to full)", rem_min);
		...
		if (backup_hour > 0)
			asprintf(&predict->sys_attr.b_ch_predict,
				 "(%dh %dm left)", backup_hour, backup_min);
		else
			asprintf(&predict->sys_attr.b_ch_predict, "(%dm left)",
				 backup_min);
	}
}
...
void shell(struct comp_info *buf)
{
	...
	while (fgets(buffer, sizeof(buffer), file) != NULL) {
		if (strncmp(buffer, "PPid", 4) == 0) {
			...
			snprintf(buffer, sizeof(buffer), "/proc/%s/comm",
				 buffer + 6);
...
void gpu(struct comp_info *buf)
{
	...
	while (fgets(buffer, sizeof(buffer), PCI_FILE) != NULL) {
		if (strncmp(buffer, device_id_str, strlen(device_id_str)) ==
		    0) {
